...
public class StreamingWorkcount {
	 ...
	 * nc -lk 8888
	 * 运行演示说明：
	 * main可以在本地run as application启动，也可以在远程spark-submit启动
	 * 使用nc工具在远程机器上临时模拟socket输入。
	 * 执行nc -lk 8888后，命令处于前端运行状态，输入一行数据后再输入一行数据，就是在模式socket数据
	 */
	public static void main(String[] args) {
		JavaStreamingContext jssc = null;
		try {
			//[2]，线程数必须大于等于2，因为ReceiverInputDStream会占用一个进程。如果设置为1，那么只会有1个线程从socket读取数据，没有多余线程来处理数据了。
			SparkConf conf = new SparkConf().setMaster("local[2]").setAppName("StreamingWorkcount");
			/*
